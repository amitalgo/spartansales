...
class DepartmentListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        query =  "select old.ID as id,(select department from leads_department where id=old.department_id) as department_name,sum(old.selling_price) as achieved,(select sum(target) from achievement_goal where department_id=old.department_id ) as target from leads_organizationleaddetails as old where old.convert=1"
...
class ProductListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        query += ") from product_productprice as pp where pp.product_id=old.product_id and pp.status=1 order by pp.id asc limit 1) as average_price from product_product as p left join leads_organizationleaddetails as old on old.product_id=p.id where old.status=1 and old.convert=1"
...
class ProductCumulativeListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        query = "select p.id,(select product_name from product_product where id=(IFNULL(p.parent_id,p.id))) as product_name,IFNULL(p.parent_id,p.id) as parent,TRUNCATE((sum(selling_price)*100/(select sum(selling_price) from leads_organizationleaddetails)),2) as sp from product_product as p left JOIN leads_organizationleaddetails as old on old.product_id=p.id group by parent"
...
class QuarterListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        query = "SELECT old.id,MONTHNAME(old.lead_closed_date) as month,(select sum(target) from achievement_goal where MONTH(old.lead_closed_date) BETWEEN MONTH(start_date) and MONTH(end_date) ) as target,sum(old.selling_price) as achieved FROM `leads_organizationleaddetails` as old where old.convert=1 "
